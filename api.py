"""
api.py — FastAPI router to trigger job-search and email workflows.
...
POST /approve/all             Alias for /email/all — same script, included for clarity
...
PYTHON = sys.executable  # same interpreter that's running uvicorn
...
async def _run_subprocess(cmd: list[str], label: str) -> int:
    """Run a command as an async subprocess, streaming stdout/stderr to the server log."""
    ...
    await proc.wait()
    print(f"[{label}] Finished with exit code {proc.returncode}")
...
async def _single_run(script: str, args: list[str], label: str):
    """Background task for single-user runs. Does NOT set _state flags."""
    try:
...
async def run_all(background_tasks: BackgroundTasks):
    """Trigger send_jobbyo.py for all users in the background."""
    if _state.full_run_active:
        raise HTTPException(
            status_code=409,
            detail=f"A full run is already in progress (started {_state.full_run_started_at.isoformat() if _state.full_run_started_at else 'unknown'}). Check /run/status.",
        )
    background_tasks.add_task(_full_run, "send_jobbyo.py", [], "run:all")
    return RunResponse(accepted=True, message="Job search started for all users. Check /run/status for progress.")
...
async def run_user(target: UserTarget, background_tasks: BackgroundTasks):
    """Trigger send_jobbyo.py for a single user (by uid or email)."""
    if not target.uid and not target.email:
        raise HTTPException(status_code=422, detail="Provide either uid or email.")

    if target.uid:
        args = ["--uid", target.uid]
        label = f"run:user:{target.uid}"
    else:
        args = ["--email", target.email]
        label = f"run:user:{target.email}"

    background_tasks.add_task(_single_run, "send_jobbyo.py", args, label)
    identifier = target.uid or target.email
    return RunResponse(accepted=True, message=f"Job search started for {identifier}.")
...
async def email_all(background_tasks: BackgroundTasks):
    ...
    return RunResponse(accepted=True, message="Approval + email run started for all users.")
...
async def email_user(target: UserTarget, background_tasks: BackgroundTasks):
    """Promote pending_review jobs and send email to a single user (by email)."""
    if not target.email:
        raise HTTPException(status_code=422, detail="Provide email (approve_jobs.py targets users by email).")
    background_tasks.add_task(_single_run, "approve_jobs.py", ["--email", target.email], f"email:user:{target.email}")
