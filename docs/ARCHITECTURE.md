...
| `company_ingestion.py` | Extracts ATS board links from this run's jobs and feeds new ones into the crawler's GCS bucket | Called inline by `send_jobbyo.py`; failures are logged, never fatal | Google Cloud Storage |
